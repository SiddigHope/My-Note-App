...
class SubCategories extends Component {
    ...
    deleteCat = async () => {
        const img = await AsyncStorage.getItem("subCats")
            ...
            [
                {
                    text: "نعم", onPress: () => {
                        ...
                        AsyncStorage.setItem("subCats", JSON.stringify(filtered))
    ...
    filterAlbums = async (item) => {
        ...
        const data = _.filter(images, obj => {
            ...
            if (obj.album == item.label) {
                ...
                // console.log(obj.item.icon)'#2196f3'
                filtered.push(obj)
    ...
    filterCats = async (item) => {
        ...
        this.props.navigation.navigate("SubCatAlbums", { cat: item.note, name: item.label, files: filtered })
    }
